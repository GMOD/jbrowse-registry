...
run_prod: node_modules js/plugins.json ## Run the server
	...
	@echo "* open http://localhost:8000/ *"
	@echo "********************************"
	./node_modules/.bin/webpack-dev-server --progress --colors --devtool source-map --optimize-minimize --host 127.0.0.1 --port 8000

build: node_modules js/plugins.json ## Compile a project for deployment
	./node_modules/.bin/webpack  --progress --colors --devtool source-map --optimize-minimize
...
deploy:
	...
	@mkdir release
	# Load required data
	@cp -Rv index.html build img demos .nojekyll  release
	# Build
	@$(MAKE) build
	# Push
	./node_modules/.bin/gh-pages -t -d release/ --repo git@github.com:gmod/jbrowse-registry.git
	# Cleanup
	@rm -rf release
